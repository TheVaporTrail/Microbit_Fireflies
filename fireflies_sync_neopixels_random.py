...
def ffRandomize():
    global gImage, gFF, gData, gCycleCount
    gCycleCount = 0
...
def ffFlashed(f):
    ...
    for k in [(f - 5) % _FF_CNT, (f - 1) % _FF_CNT, (f + 5) % _FF_CNT, (f + 1) % _FF_CNT]:
        if gFF[k][_TMR] > 0 and gFF[k][_TMR] < BLINK_INTERVAL:
            gFF[k][_TMR] -= int((BLINK_INTERVAL - gFF[k][_TMR]) * TIMER_NUDGE) 
            if gFF[k][_TMR] < 0:
                gFF[k][_TMR] = 0
        if gClr != -1 and gClr != gData[_FF_CNT + k]:
            gData[_FF_CNT + k] += int(0.1 * (gClr - gData[_FF_CNT + k]))
...
while True:
    ...
    elapsedTime = running_time() - gFF[f][FF_TS]
    if elapsedTime < 0:
        elapsedTime = 0
    gFF[f][FF_TS] = running_time()
    ...
    if update:
        ...
        if c < 40:
            ...
            g = 120 - c
            b = 0
        gImage[f] = (int(br*3*r/40), int(br*3*g/40), int(br*3*b/40))
        pushUpdate = True
